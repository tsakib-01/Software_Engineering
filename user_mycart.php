...
$sql = "SELECT * FROM cart_table WHERE cart_email = '$userEmail'";
...
$sql = "SELECT profileIMG FROM users WHERE email = ?";  // Adjust the WHERE clause as needed
$stmt = $conn->prepare($sql);
      ...
      <ul class="space-y-4">
        ...
        <li>
          <button onclick="showContent('wishlistContent')" class="w-full py-2 px-4 bg-blue-700 rounded hover:bg-blue-600 focus:outline-none">
           ...
           <li>
          <button onclick="showContent('reservationContent')" class="w-full py-2 px-4 bg-blue-700 rounded hover:bg-blue-600 focus:outline-none">
            Booking Reservation
          </button>
        </li>
...
<p class="text-gray-600">Price: 
  <span class="font-semibold">
    <?php 
      if (!empty($item['cart_discount']) && $item['cart_discount'] > 0) {
          echo htmlspecialchars($item['cart_discount']) . " Tk.";
      } else {
          echo htmlspecialchars($item['cart_price']) . " Tk.";
      }
    ?>
  </span>
</p>
...
$cartItems = [];  // Assuming you have this array to store cart items
...
while ($row = $result->fetch_assoc()) {
    ...
    // Use discount price if it's valid (greater than 0 and less than cart_price), else use cart_price
    if (isset($row['cart_discount']) && $row['cart_discount'] > 0 && $row['cart_discount'] < $row['cart_price']) {
        $itemPrice = $row['cart_discount'];
    } else {
        $itemPrice = $row['cart_price'];
    }
...
// Assume $email holds the logged-in user's email

// Assuming $conn is your DB connection and $email is logged-in user's email

$stmtReviews = $conn->prepare("
    SELECT r.reviewID, r.productID, r.rating, r.comment, r.created_at, r.email, p.titles AS productTitle 
    FROM reviews r 
    JOIN product_table p ON r.productID = p.productID
    WHERE r.email = ?
    ORDER BY r.created_at DESC
");
$stmtReviews->bind_param("s", $email);
$stmtReviews->execute();
...
<?php if (count($cartItems) > 0): ?>
  ...
  <p class="text-xl font-semibold text-gray-700">Tk.<?php echo number_format($totalPrice, 2); ?></p>
</div>
...
<?php foreach ($userReviews as $review): ?>
  <div class="bg-gray-50 p-4 rounded-lg shadow-md">
    <p class="text-lg font-semibold text-gray-700">Product: <?= htmlspecialchars($review['productTitle']) ?></p>
    <p class="text-yellow-500">
      <?= str_repeat('★', (int)$review['rating']) ?> (Rating)
    </p>
    <p class="mt-2 text-gray-600"><?= nl2br(htmlspecialchars($review['comment'])) ?></p>
    <small class="text-gray-400"><?= $review['created_at'] ?></small>

    <?php if (isset($_SESSION['email']) && strtolower($_SESSION['email']) === strtolower($review['email'])): ?>
...
<?php else: ?>
  <p class="text-center text-gray-600">You have not submitted any reviews yet.</p>
<?php endif; ?>
...
if (!empty($wishlistItems)) {
    ...
    $query = "SELECT productID, productIMG, category, subtitles, titles, description, price, discount, quantity 
...
<!-- For showing comment section from welcome page directly using JS -->
<script>
  ...
  window.addEventListener('DOMContentLoaded', () => {
    if (getUrlParam('showComments') === '1') {
      const commentsDiv = document.getElementById('commentsContent');
      if (commentsDiv) {
        commentsDiv.style.display = 'block';
        // Scroll smoothly to the comments div
...
<div id="wishlistContent" class="content-section p-4 bg-gray-100 rounded-lg shadow-lg" style="display:block;">
  ...
  <?php
    if (!empty($product['discount']) && $product['discount'] > 0) {
        echo htmlspecialchars($product['discount']) . " Tk.";
    } else {
        echo htmlspecialchars($product['price']) . " Tk.";
    }
  ?>
</td>
 ...
 <a href="product.php?id=<?= $product['productID']; ?>&first_name=<?= urlencode($firstName); ?>&email=<?= urlencode($email); ?>" 
   class="inline-block text-yellow-400 hover:text-yellow-600 bg-transparent border-none px-4 py-2 rounded">
   Preview
</a>
...
<?php if (isset($_GET['deleted'])): ?>
  <div class="bg-green-100 text-green-800 p-2 mb-4 rounded">Reservation canceled successfully.</div>
<?php elseif (isset($_GET['error'])): ?>
  <div class="bg-red-100 text-red-800 p-2 mb-4 rounded">Could not cancel reservation.</div>
<?php endif; ?>
...
<div class="mt-10">
  <h2 class="text-2xl font-bold text-gray-800 mb-6">Your Reservations</h2>
      ...
      <?php foreach ($reservations as $res): ?>
        <div class="bg-white p-6 rounded-xl shadow-md border border-gray-200">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-700">
            <p><span class="font-semibold">Date:</span> <?= htmlspecialchars($res['reservation_date']) ?></p>
            <p><span class="font-semibold">Time:</span> <?= htmlspecialchars($res['reservation_time']) ?></p>
            <p><span class="font-semibold">People:</span> <?= htmlspecialchars($res['number_of_people']) ?></p>
            <p><span class="font-semibold">Phone:</span> <?= htmlspecialchars($res['phone']) ?></p>
            <p class="md:col-span-2"><span class="font-semibold">Message:</span> <?= nl2br(htmlspecialchars($res['message'])) ?></p>
          </div>

          <div class="flex items-center justify-between mt-4">
            <p class="text-sm text-gray-500">Booked on: <?= date('F j, Y, g:i a', strtotime($res['created_at'])) ?></p>
            <form action="delete_reservation.php" method="POST" onsubmit="return confirm('Cancel this reservation?');">
              <input type="hidden" name="reservationID" value="<?= $res['reservationID'] ?>">
              <button type="submit" class="text-red-500 hover:text-red-700 text-sm font-medium transition">Cancel</button>
            </form>
          </div>
        </div>
      <?php endforeach; ?>
  ...
  <?php else: ?>
    <p class="text-gray-500 italic">You have no reservations at the moment.</p>
  <?php endif; ?>
</div>
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
      ...
      $updateQuery = "UPDATE users SET fname = ?, lname = ?, address = ?, password = ?, profileIMG = ? WHERE email = ?";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['deleteProfileImg']) && !empty($user['profileIMG'])) {
        ...
        $updateQuery = "UPDATE users SET profileIMG = NULL WHERE email = ?";
        $updateStmt = $conn->prepare($updateQuery);
